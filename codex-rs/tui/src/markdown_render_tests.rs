...
fn inline_code() {
    ...
    let expected =
        Line::from_iter(["Example of ".into(), "Inline code".light_blue()]).into();
    assert_eq!(text, expected);
}
...
fn nested_five_levels_mixed_lists() {
    ...
    let text = render_markdown_text(md);
    let expected = Text::from_iter([
        Line::from_iter(["1. ".green(), "First".into()]),
        Line::from_iter([
            Span::styled("    • ", Style::default().green()),
            "Second level".into(),
        ]),
        Line::from_iter(["        1. ".green(), "Third level (ordered)".into()]),
        Line::from_iter([
            Span::styled("            • ", Style::default().green()),
...
fn html_in_tight_ordered_item_soft_breaks_with_space() {
    ...
    let text = render_markdown_text(md);
    let expected = Text::from_iter([
        Line::from_iter(["1. ".green(), "Foo".into()]),
...
fn ordered_item_continuation_paragraph_is_indented() {
    ...
    let text = render_markdown_text(md);
    let expected = Text::from_iter([
        Line::from_iter(["1. ".green(), "Intro".into()]),
        Line::default(),
