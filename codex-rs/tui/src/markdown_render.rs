    ...
    I: Iterator<Item = Event<'a>>,
{
    ...
    fn handle_event(&mut self, event: Event<'a>) {
        match event {
            ...
            Event::Rule => {
                ...
                self.push_line(Line::from(vec![Span::styled("———", style)]));
    ...
    fn code(&mut self, code: CowStr<'a>) {
        ...
        let span = Span::from(code.into_string()).light_blue();
        self.push_span(span);
    }
    ...
    fn start_item(&mut self) {
        ...
        let marker = if let Some(last_index) = self.list_indices.last_mut() {
            match last_index {
                None => {
                    let marker_text = format!("{}• ", " ".repeat(width - 1));
                    let style = Style::default().light_blue();
                    Some(vec![Span::styled(marker_text, style)])
                }
                Some(index) => {
                    *index += 1;
    ...
    fn end_codeblock(&mut self) {
        ...
        if !code.is_empty() {
            let highlighted = self.highlight_code_block(&code, lang.as_deref());
    ...
    fn highlight_code_block(&self, code: &str, lang: Option<&str>) -> Vec<Line<'static>> {
        ...
        let normalized = lang.map(|s| s.to_ascii_lowercase());
        match normalized.as_deref() {
            Some("bash") | Some("sh") => highlight_bash_to_lines(code),
            Some("rust") | Some("rs") => crate::render::highlight::highlight_rust_to_lines(code),
            Some("python") | Some("py") => {
                crate::render::highlight::highlight_python_to_lines(code)
            }
            Some("javascript") | Some("js") => {
                crate::render::highlight::highlight_javascript_to_lines(code)
            }
            Some("typescript") | Some("ts") | Some("tsx") => {
                crate::render::highlight::highlight_typescript_to_lines(code)
            }
            Some("html") | Some("htm") => crate::render::highlight::highlight_html_to_lines(code),
            Some("css") => crate::render::highlight::highlight_css_to_lines(code),
            Some("sql") => crate::render::highlight::highlight_sql_to_lines(code),
            Some("json") => crate::render::highlight::highlight_json_to_lines(code),
            _ => code
                .split('\n')
